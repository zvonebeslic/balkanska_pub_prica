(() => {
  ...
  function tick() {
    ...
    if (running && isActive()) activeSeconds += Math.max(0, (now - lastTick) / 1000);
    lastTick = now;
  }
  ...
  async function flush() {
    ...
    if (!playId) return;
    try {
      await client.from('quiz_plays').update({
        duration_seconds: Math.floor(activeSeconds),
        correct_answers: correct,
        wrong_answers: wrong,
        viewed_questions: viewed,
        scrolled_questions: scrolled,
        learn_more_clicks: learnMore,
        share_clicks: shares
      }).eq('id', playId);
  ...
  async function start(topic) {
    ...
    try {
      ...
      const { data, error } = await client.from('quiz_plays').insert({
        user_id: userId,
  ...
  async function saveQuestion(card, type) {
    ...
    try {
      await client.from('quiz_answers').insert({
        ...
        question_id: null,
        question_text: text,
        question_topic: activeTopic,
        question_type: 'scroll',
        quiz_mode: 'scroll',
        selected_theme: activeTopic,
        user_answer: type === 'answered' ? input : null,
        correct_answer: null,
        matched_answer: null,
        is_correct: isCorrect,
        is_exact: false,
        was_skipped: false,
        was_scrolled: type === 'scrolled',
        was_viewed: !!state.viewed,
        match_reason: type === 'scrolled' ? 'scrolled' : 'scroll_answer',
        language: 'hr',
  ...
  const observer = new IntersectionObserver(entries => {
    for (const entry of entries) {
      ...
      if (entry.intersectionRatio >= 0.6) {
        ...
        if (!s.viewed && !s.timer) {
          s.timer = setTimeout(() => {
            if (document.body.contains(card) && !s.viewed) {
              s.viewed = true;
              viewed += 1;
              flush();
            }
        ...
        if (s.entered && !s.answered && !s.scrolled) {
          s.scrolled = true;
          scrolled += 1;
          saveQuestion(card, 'scrolled');
          flush();
        }
      }
  ...
  function observeCards() {
    document.querySelectorAll('.q').forEach(card => {
      if (!cardState.has(card)) {
        cardState.set(card, { entered:false, viewed:false, answered:false, scrolled:false, timer:null });
        observer.observe(card);
      }
    });
  }
  new MutationObserver(observeCards).observe(document.getElementById('feed'), { childList:true });
  observeCards();
  ...
  document.addEventListener('click', e => {
    ...
    else if (topic) start(topic.textContent.trim());
    ...
    if (e.target.closest('.ok')) {
      setTimeout(() => {
        ...
        const result = card.querySelector('.result');
        if (!result?.classList.contains('good') && !result?.classList.contains('bad')) return;
        s.answered = true;
        cardState.set(card, s);
        if (result.classList.contains('good')) correct += 1; else wrong += 1;
